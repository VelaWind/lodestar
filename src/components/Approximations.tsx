...
export function Approximations({ items }: { items: string[] }) {
          ...
          >
            <ul className="space-y-2.5 border-t border-edge-soft px-4 py-3 font-prose text-sm leading-relaxed text-ink-dim">
              {items.map((item, i) => (
                <li
                  key={i}
                  className="relative pl-4 before:absolute before:left-0 before:top-[0.65em] before:h-1 before:w-1 before:rounded-full before:bg-ember/60"
                >
                  {item}
                </li>
              ))}
